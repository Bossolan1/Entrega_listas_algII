...
int main()
{
    ...
    for(int i = 0; i < 6; i++)
    {
        ...
        strcpy(lista_atleta[i].esporte, esporte_aleatorio);
    ...
    qsort(atletas, 6, sizeof(atleta), comparar_por_idade);
    printf("Lista de atletas:\n");
    for(int i =0; i < 6; i++)
    {
        printf("Nome   : %s\n", lista_atleta[i].nome);
        printf("esporte: %s\n", lista_atleta[i].esporte);
        printf("Idade  : %d\n",lista_atleta[i].idade);
        printf("Altura : %.2f\n",lista_atleta[i].altura);
    }
    return 0;
}
